...
def date_converter(months,date):
	day_pos = date.find('/')
	year_pos = date.find('/',day_pos+1)
	month = date[0:day_pos]
	day = date[day_pos+1:year_pos]
	year = date[year_pos+1:]
	proper_date = day + " " + months[int(month)] + " " + year
	return proper_date
